...
class SQSHandler:
    def __init__(self):
        ...
        self.assistant_queue_url = AWS_CONFIG['assistant_queue_url']
    ...
    async def send_assistant_message(
        ...
        try:
            ...
            # SQS에 메시지 전송 (assistant_queue_url 사용)
            response = self.sqs_client.send_message(
                QueueUrl=self.assistant_queue_url,
                MessageBody=json.dumps(message_body)
            )
            
            logger.info(f"Assistant message sent to SQS: {message_body}")
            return response
        
        except ClientError as e:
            logger.error(f"Failed to send assistant message to SQS: {str(e)}")
            raise 