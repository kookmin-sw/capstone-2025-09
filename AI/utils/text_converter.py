...
4. 이 텍스트에 욕설이 포함되어 있는지 판단합니다.
   - 욕설 검출은 반드시 '어절 단위'로 수행하며, 해당 어절이 문맥상 명백한 욕설로 사용된 경우에만 욕설로 판단합니다.
   - 단순히 특정 단어의 일부와 형태가 같다고 해서 욕설로 판단해서는 안 됩니다. (예: "시발"은 욕설로 필터링될 수 있지만, "시발점", "시발역"과 같이 다른 의미로 사용되는 전체 어절은 욕설이 아닙니다.)
   - 특히, 일반적인 단어나 고유명사에 포함된 글자가 욕설과 유사하더라도, 해당 어절 전체가 욕설의 의미로 사용되지 않는 한 욕설로 간주하지 마십시오.
5. **최종 출력은 다음 JSON 형식의 문자열로만 출력하십시오:**
   ...
   "Text" 키의 값은 규칙에 따라 변환되고 분할된 문장들의 리스트입니다.
   - 만약 텍스트가 안전하지 않아 "safe"가 False라면, "Text" 리스트의 문장들 내에서 **검출된 욕설 어절은 `**욕설어절**` 형식으로 양쪽에 별표 두 개를 붙여 표시**하십시오. 나머지 처리(숫자 변환, 문장 분할)는 동일하게 적용합니다.
...
예시 3. 욕설 어절이 포함된 경우
입력: "그가 순간 분노하여 시발을 외쳤다. 그리고 10번이나 반복했다."
출력: {
    "safe":false,
    "Text":["그가 순간 분노하여 **시발**을 외쳤다.", "그리고 열번이나 반복했다."]
    }
