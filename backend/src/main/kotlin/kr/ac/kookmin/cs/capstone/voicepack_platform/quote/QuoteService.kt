...
class QuoteService(
    ...
    private fun createOpenAiPrompt(emotion: String, category: QuoteCategory): String {
        ...
        return "현재 감정 상태는 '${emotion}'입니다. 이 감정에 어울리는 '${categoryDescription}'의 유명한 명언 한 구절과 그 명언을 말한 사람을 웹 검색을 통해 찾아보고, 검증된 정보를 바탕으로 알려주세요. 응답 형식은 '{말한 사람}은 이렇게 말했습니다. {명언 내용}' 형태로 한글로 답해주세요. 특수문자는 제외합니다. 생셩 결과 끝에 말한 사람을 다시 붙이지 마세요. 적절한 결과가 없다면 일반적인 명언을 생성하고, '누군가 이렇게 말했습니다. {명언 내용}' 형식으로 출력합니다." 
    }
